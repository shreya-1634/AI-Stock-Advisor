...
class Visualization:
    ...
    def plot_prediction_chart(self, historical_df: pd.DataFrame, predicted_data) -> go.Figure:
        if historical_df.empty or predicted_data.empty:
        ...
        # Support both DataFrames (new format) and Series (old format)
        if isinstance(predicted_data, pd.DataFrame):
            pred_y = predicted_data['Predicted Close']
        else:
            pred_y = predicted_data.values

        fig.add_trace(go.Scatter(x=predicted_data.index, y=pred_y, mode='lines+markers', name='Predicted Close', line=dict(color='orange', dash='dash'), marker=dict(symbol='circle', size=8)))
