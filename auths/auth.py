...
class AuthManager:
    ...
    def signup_ui(self):
        ...
        with st.form("signup_form"):
            email = st.text_input("Email", key="signup_email_input").strip().lower()
    ...
    def login_ui(self):
        ...
        with st.form("login_form"):
            email = st.text_input("Email", key="login_email_input").strip().lower()
                ...
                if user_data:
                    if self.password_utils.verify_password(password, user_data['password_hash']):
                        if user_data.get('is_verified', False):
                            ...
                            self.user_manager.log_activity(email, "login", "Successful login.")
                        ...
                        self.user_manager.log_activity(email, "login_failed", "Invalid password.")
                else:
                    st.error("Invalid email or password.")
                    self.user_manager.log_activity(email, "login_failed", "Email not found.")
    ...
    def reset_password_ui(self):
        ...
        if not st.session_state['reset_email_sent']:
            ...
            with st.form("request_reset_form"):
                email = st.text_input("Email", key="reset_request_email").strip().lower()
            ...
            with st.form("reset_password_form"):
                ...
                if reset_button:
                    ...
                    if not new_password or not confirm_new_password:
                        ...
                        if self.user_manager.update_password(email, hashed_new_password):
                            ...
                            # Note: Ensure this matches the routing variable used in your app.py
